...
class Plotter:
    ...
    def plot_vg(self, frame='alpha-beta', ax=None, line_style='solid'):
        """
        Plot grid voltage vg in specified reference frame.
        
        Parameters
        ----------
        frame : str, optional
            Reference frame for plotting ('alpha-beta', 'abc', 'dq').
            Default: 'alpha-beta'
        ax : matplotlib.axes.Axes, optional
            Axis to plot on. If None, a new figure is created.
        line_style : str, optional
            Line style for the plotted voltage curves.
            Default: 'solid'

        Examples
        --------
        >>> plotter.plot_vg(frame='abc')
        >>> plotter.plot_vg(frame='alpha-beta')
        >>> plotter.plot_vg(frame='alpha-beta', ax=ax)
        """
        if not hasattr(self.data.sys, 'vg'):
            raise KeyError(
                "No 'vg' attribute in system. This system is not a grid.")
        ...
        if make_fig:
            t_idx, t_plot, axs = self._get_time_mask_and_plot(
                t_sys, self.t_start, self.t_end, 1)
            ax = axs[0]
        else:
            t_idx, t_plot = self._get_time_mask(t_sys, self.t_start,
                                                self.t_end)

        # Convert to requested frame
        if frame == 'alpha-beta':
            vg_plot = vg[t_idx, :]
            components = [r'\alpha', r'\beta']
        elif frame == 'abc':
            vg_plot = np.array([alpha_beta_2_abc(v) for v in vg[t_idx, :]])
            components = ['a', 'b', 'c']
        elif frame == 'dq':
            theta = np.arctan2(vg[t_idx, 1], vg[t_idx, 0])
            vg_plot = np.array(
                [alpha_beta_2_dq(v, th) for v, th in zip(vg[t_idx, :], theta)])
            components = ['d', 'q']
        else:
        ...
        for j, comp in enumerate(components):
            label = fr"$v_{{g,{comp}}}$"
            color = self.phase_colors[j] if j < len(
                self.phase_colors) else None
            ax.plot(t_plot,
                    vg_plot[:, j],
                    label=label,
                    color=color,
        ...
        # Reference plotting removed per user request

        ax.set_ylabel(r"Grid voltage $v_g$ [p.u.]")
        ax.grid(True)
        ax.legend(loc='upper left')
        ax.set_xlim([
            self.t_start, self.t_end if self.t_end is not None else t_sys[-1]
        ])
        ax.set_xlabel("Time [s]")
        if make_fig:
            plt.tight_layout()

    def plot_v_pcc(self, frame='alpha-beta', ax=None, line_style='solid'):
        """
        Plot point of common coupling voltage v_pcc in specified reference frame.
        ...
        Examples
        --------
        >>> plotter.plot_v_pcc(frame='abc')
        >>> plotter.plot_v_pcc(frame='dq')
        """
        if not hasattr(self.data.sys, 'v_pcc'):
            raise KeyError(
                "No 'v_pcc' attribute in system. Make sure v_pcc is calculated and stored."
            )

        t_sys = self.data.sys.t
        v_pcc = self.data.sys.v_pcc
        make_fig = ax is None
        if make_fig:
            t_idx, t_plot, axs = self._get_time_mask_and_plot(
                t_sys, self.t_start, self.t_end, 1)
        ...
        # Convert to requested frame
        if frame == 'alpha-beta':
            v_pcc_plot = v_pcc[t_idx, :]
            components = [r'\alpha', r'\beta']
        elif frame == 'abc':
            v_pcc_plot = np.array(
                [alpha_beta_2_abc(v) for v in v_pcc[t_idx, :]])
            components = ['a', 'b', 'c']
        elif frame == 'dq':
            theta = np.arctan2(v_pcc[t_idx, 1], v_pcc[t_idx, 0])
            v_pcc_plot = np.array([
                alpha_beta_2_dq(v, th)
                for v, th in zip(v_pcc[t_idx, :], theta)
            ])
            components = ['d', 'q']
        else:
            raise ValueError(f"Unknown frame: {frame}")
        ...
        ax.set_ylabel(r"PCC voltage $v_{\mathrm{pcc}}$ [p.u.]")
        ax.grid(True)
        ax.legend(loc='upper left')
        ax.set_xlim([
            self.t_start, self.t_end if self.t_end is not None else t_sys[-1]
    ...
    def plot_vg_and_v_pcc(self, frame='alpha-beta'):
        """
        Plot vg and v_pcc on the same axes in one figure.
        ...
        >>> plotter.plot_vg_and_v_pcc(frame='dq')
        """
        if not hasattr(self.data.sys, 'vg'):
            raise KeyError(
                "No 'vg' attribute in system. This system is not a grid.")
        ...
        self._register_figure(fig)
        self.plot_vg(frame=frame, ax=ax, line_style='solid')
        self.plot_v_pcc(frame=frame, ax=ax, line_style='--')
        ax.set_title(r"Grid voltage $v_g$ and PCC voltage $v_{\mathrm{pcc}}$")
        ax.set_xlabel("Time [s]")
        ax.legend(loc='upper left')
        plt.tight_layout()
