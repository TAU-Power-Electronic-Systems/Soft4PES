...
class TrianguarCarrier:
    ...
    def __init__(self, Ts, Ts_sim, lower_limit, upper_limit, initial_val):
        """
        Initialize a TrianguarCarrier instance.
        ...
        self.direction = -1
        self.step = (upper_limit - lower_limit) * (Ts_sim / Ts)

    def update_carrier_value(self):
        """
...
class CarrierPWMOld:
    ...
    def __init__(self, Ts_ctr, Ts_sim, nl):
        ...
        if nl == 2:
            self.lower_carrier = TrianguarCarrier(Ts_ctr, Ts_sim, -1, 1, 1)
            self.upper_carrier = None
            self.get_switch_positions = self.two_level_comparison

        elif nl == 3:
            self.lower_carrier = TrianguarCarrier(Ts_ctr, Ts_sim, -1, 0, 0)
            self.upper_carrier = TrianguarCarrier(Ts_ctr, Ts_sim, 0, 1, 1)
    ...
    def two_level_comparison(self, u_abc_ref):
        ...
        carrier_value = self.lower_carrier.value

        # Update the carrier value
        self.lower_carrier.update_carrier_value()

        # Generate the three-phase switch positions
        u_abc = np.zeros(3)
        for i in range(3):
            u_abc[i] = 1 if u_abc_ref[i] > carrier_value else -1
        return u_abc
    ...
    def three_level_comparison(self, u_abc_ref):
        ...
        # Get the carrier values
        lower_carrier_value = self.lower_carrier.value
        upper_carrier_value = self.upper_carrier.value

        # Update the carrier values
        self.lower_carrier.update_carrier_value()
        self.upper_carrier.update_carrier_value()

        # Generate the three-phase switch positions
        u_abc = np.zeros(3)
        for i in range(3):
            if u_abc_ref[i] > upper_carrier_value:
                u_abc[i] = 1
            elif u_abc_ref[i] < lower_carrier_value:
                u_abc[i] = -1
            else:
                u_abc[i] = 0
        return u_abc
