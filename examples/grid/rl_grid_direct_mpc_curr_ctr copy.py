...
P_ref_seq = Sequence(np.array([0, 0.1, 0.1, 1]), np.array([1, 1, 0.5, 0.5]))
V_ref_seq = Sequence(np.array([0, 1]), np.array([1, 1]))
...
conv = model.conv.Converter(v_dc_SI=2 * 5200, nl=3, base=base)
...
ctrSys = common.ControlSystem(control_loops=[rfpsc],
                              ref_seq=ref_seq,
                              Ts=100e-6)
